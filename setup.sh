#!/usr/bin/env bash
# Split MIT-BIH PhysioNet zip into GitHub-safe chunks (<100 MiB).
...
# Stay under GitHub's 100 MiB hard limit. Default 40M so the ~74 MiB PhysioNet
# zip becomes multiple shards (95M would leave a single ~74 MiB part).
CHUNK_SIZE="${CHUNK_SIZE:-40M}"
PARTS_DIR="${PARTS_DIR:-parts}"
PART_PREFIX="mitdb-1.0.0.zip.part"
ZIP_NAME="mitdb-1.0.0.zip"
EXTRACT_DIR="${EXTRACT_DIR:-extracted}"

log() { printf '[mit-bih] %s\n' "$*"; }
die() { printf '[mit-bih] ERROR: %s\n' "$*" >&2; exit 1; }
...
list_source_volumes() {
  local vols=()
  local f
  for f in "$ROOT"/mitdb-1.0.0.zip.[0-9][0-9][0-9]; do
    [[ -f "$f" ]] || continue
    vols+=("$f")
  done
  if ((${#vols[@]} > 0)); then
...
join_sources_to_zip() {
  local out="$1"
  local -a vols=()
  mapfile -t vols < <(list_source_volumes) || die "no source archive found (expected $ZIP_NAME or mitdb-1.0.0.zip.001...)"
...
split_zip() {
  ...
  {
    echo "# MIT-BIH GitHub-safe zip shards"
...
cleanup_sources_after_prepare() {
  # Drop the full zip / split volumes once GitHub-safe parts exist (even if <100 MiB).
  local -a vols=()
...
cmd_prepare() {
  ...
  local tmp
  tmp="$(mktemp "$ROOT/mitdb.zip.XXXXXX")"
...
Usage: bash setup.sh <prepare|restore|verify>
  ...
  prepare  Join $ZIP_NAME / mitdb-1.0.0.zip.001+ , verify, split into $PARTS_DIR/ ($CHUNK_SIZE each)
